    ...
    if protocol:
        ...
        if "akmi" in medical_files:
            try:
                ...
                complex_markers = ["балон", "баллон", "ангіопластик", "пухлин", "зоб", "уламк", "стороннє тіло", "міксома", "тератома", "метастаз", "лімфовуз"]
                ...
                with st.spinner("Пошук оптимальної комбінації кодів..."):
                    ...
                    2. АОРТА ТА FET:
                       ...
                       - Якщо є протезування аорти (висікання аорти і заміна протезом) - кодуй саме протезування. Відновлення - тільки якщо виконано екзопротезування (окутування аорти) або бандажування аорти.
                    ...
                    6. СТОП-ЛИСТ (ІГНОРУВАТИ ВЗАГАЛІ):
                       - Тимчасові епікардіальні електроди (2 дроти), якщо не вказано, що навʼязано ритм кардіостимулятора.
                       - Дефібриляція в межах відновлення ритму серця після штучного кровообігу.
